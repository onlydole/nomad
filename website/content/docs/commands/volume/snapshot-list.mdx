...
```plaintext
nomad snapshot list [-plugin plugin_id]
```

The `snapshot list` command returns a list of snapshots along with their
...
- `-plugin_id`: Display only snapshots managed by a particular [CSI
