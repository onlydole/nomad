...
```plaintext
nomad snapshot create [volume] [snapshot_id]
```

The `snapshot create` command requires a volume ID or prefix. If there is an
exact match based on the provided volume ID or prefix, then the specific
...
If an optional snapshot ID is provided, the argument will be passed to the CSI
plugin to be used as ID of the resulting snapshot. Not all plugins accept this
ID and it may be ignored.
